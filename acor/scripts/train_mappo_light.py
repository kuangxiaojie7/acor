...
from acor.runners.mappo_light_trainer import MAPPOTrainerLight
from acor.utils.config import build_argparser, load_config, parse_unknown_args


def main() -> None:
    parser = build_argparser("Train MAPPO-Light baseline on PettingZoo environments.")
    ...
    trainer = MAPPOTrainerLight(config=config, output_dir=str(output_dir) if output_dir else None)
    trainer.train()
