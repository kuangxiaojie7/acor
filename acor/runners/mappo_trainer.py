...
class MAPPOTrainer:
    """
    Parameter shared MAPPO baseline trainer.
    """
    ...
    def train(self) -> None:
        ...
        optimizer = torch.optim.AdamW(policy.parameters(), lr=self.config["train"]["lr"])

        buffer = RolloutBuffer(
            horizon=self.config["train"]["rollout_length"],
            num_envs=exp_cfg["num_envs"],
        ...
        for update_idx in range(total_updates):
            ...
            metrics = self._update(policy, optimizer, buffer, self.config["train"])
    ...
    def _update(self, policy: torch.nn.Module, optimizer: torch.optim.Optimizer, buffer: RolloutBuffer, train_cfg: Dict) -> Dict[str, float]:
